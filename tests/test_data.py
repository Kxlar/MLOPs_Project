...
# Import modules from data.py
# Make sure data.py is accessible in the PYTHONPATH or in the same directory
from data import (
    MVTecDataset,
    build_transform,
    build_dataloaders,
...
def mock_mvtec_data(tmp_path):
    ...
    for i in range(2):
        ...
        create_dummy_image(
            root / class_name / "test" / "broken_large" / img_name,
            color=(255, 0, 0)
        )
...
def test_save_augmented_dataset(mock_args, tmp_path):
    ...
    expected_output_dir = (
        output_root
        / f"{mock_args.class_name}_augmented"
        / "train"
        / "good"
    )
